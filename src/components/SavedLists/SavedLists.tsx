...
import type { SavedListProps } from "./SavedLists.types";
...
export const SavedLists = ({onClose}: SavedListProps) => {
    ...
    const handleSendWhatsappData = (listName: string) => {
        ...
        savedLists.find(list => list.name === listName)?.items.map((item => `Item: ${item.name} - Quantide: ${item.quantity}`)).join("\n")

        const whatsAppUrl = `https://api.whatsapp.com/send?phone=+5551999212494&text=${encodeURIComponent(message)}`
